...
def design_matrix(x, degree):
    ...
    exp = np.array(range(degree + 1))
    X =  np.repeat(x, exp.size).reshape(x.size, exp.size)
    j = 0

    for j in range(0, x.size):
        X[j] = np.power(X[j], exp)
...
def train(x, y, degree):
    ...
    theta_opt = np.dot(pinv(design_matrix(x, degree)), y)
...
def compute_error(theta, degree, x, y):
    ...
    err = np.mean((np.dot(design_matrix(x, degree), theta) - y) ** 2)
