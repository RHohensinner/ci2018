...
def get_centers_and_sigma(n_centers):
    ...
    sigma = 2. / n_centers  # TODO: Change me
...
def design_matrix(x, centers, sigma):
    ...
    tmp_m.append(all_ones)
     
    for j in centers:
        tmp_n = []

        for k in x:
            tmp_n.append(math.exp( - (k - j) ** 2 / (2 * sigma ** 2)))
...
def train(x, y, n_centers):
    ...
    theta_opt = np.dot(np.linalg.pinv(design_matrix(x, centers, sigma)), y)
...
def compute_error(theta, n_centers, x, y):
    ...
    centers, sigma = get_centers_and_sigma(n_centers)
    y_calc = np.dot(design_matrix(x, centers, sigma), theta)
    err = np.mean((y_calc - y) ** 2)
